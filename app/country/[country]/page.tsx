...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${name} 여행 유튜브 영상을 분석한 AI 여행 계획 모음`,
  };
}

async function getPlansByCountry(country: string): Promise<PlanCardData[]> {
  try {
...
export default async function CountryPage({ params }: Props) {
  const { country } = await params;
  const name = decodeURIComponent(country);
  const plans = await getPlansByCountry(name);
          ...
          <div className="container mx-auto px-4 md:px-6">
            ...
            {plans.length > 0 ? (
              ...
              <div className="text-center py-20 text-muted-foreground">
                ...
                <p className="text-lg">아직 {name} 여행 계획이 없습니다.</p>
