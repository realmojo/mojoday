"""
스킵된 16개 포스트의 내부 링크 UI 업그레이드
WP 렌더링으로 </div> 구조가 달라진 경우 처리
"""
...
# 스킵된 16개
SKIP_IDS = [37506, 37514, 37515, 37516, 37517, 37518, 37519, 37525, 37529, 37532, 37536, 37537, 37541, 37544, 37546, 37547]
...
def main():
    ...
    for pid in SKIP_IDS:
        ...
        # 블록 안의 모든 링크(href + title) 추출
        # block_start부터 5000자 범위에서 찾기
        search_area = content[block_start:block_start+5000]
        links = re.findall(r'href="(https://devupbox\.com/[^"]+)"[^>]*>(?:→|📄)?\s*(.+?)</a>', search_area)
        ...
        # 블록 끝 찾기 - 마지막 </a> 이후 가장 가까운 </div> (1개든 2개든)
        last_link_end = 0
        for m in re.finditer(r'</a>', search_area):
            last_link_end = m.end()

        # last_link_end 이후 연속 </div> 찾기
        remaining = search_area[last_link_end:]
        div_match = re.match(r'(\s*</div>)+', remaining)
        if div_match:
            block_end_relative = last_link_end + div_match.end()
        else:
            block_end_relative = last_link_end

        block_end = block_start + block_end_relative
